...
class CrossEntropyCriterion(Module):
    def __init__(self, *args):
        ...
        self.name = "CrossEntropyCriterion"
    ...
    def logsoftmax(self, z):
        K = np.amax(z, axis=1).reshape((-1, 1))
        exp_z = np.exp(z - K)
        return np.log(1e-3 + exp_z / exp_z.sum(axis=1).reshape((-1, 1)))

    def forward(self, *args, **kwargs):
        #print ('f-prop called for module %s' % self.name)
        super(CrossEntropyCriterion, self).forward(*args, **kwargs)
        self.answers = self.softmax(self.input)
        self.output = - np.multiply(self.target, self.logsoftmax(self.input)).sum().sum()
        return self.output
    
    def backward(self, *args, **kwargs):
        #print ('b-prop called for module %s' % self.name)
    	super(CrossEntropyCriterion, self).backward(*args, **kwargs)
    ...
    def local_gradient(self, inputs, target=None, eps=1e-3, tol=1e-3):
        ...
        #===========================
        grad_an = self.softmax(inputs) - target
        #===========================
